...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        df_aux = spacex_df[spacex_df['Launch Site']==entered_site]['class'].value_counts().to_frame().reset_index()
        fig = px.pie(df_aux, values='count',
        names = 'class',
...
def get_scatter_chart(entered_site, payload_lims):
    filtered_df = spacex_df
    low, high = payload_lims
    if entered_site == 'ALL':
        ...
        df_aux2 = spacex_df[spacex_df['Launch Site']==entered_site]
        mask = (df_aux2['Payload Mass (kg)'] > low) & (df_aux2['Payload Mass (kg)'] < high)
        fig2 = px.scatter(df_aux2[mask], x='Payload Mass (kg)',y='class', color='Booster Version Category',
